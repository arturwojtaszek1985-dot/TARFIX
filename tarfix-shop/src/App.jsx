...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .product-emoji{font-size:3rem;text-align:center;padding:20px 0 14px;background:#f2fbf7;border-bottom:1px solid #cdeee0}
    ...
    <>
      ...
      <div className="app">
        <nav className="navbar">
          <div className="navbar-inner">
            ...
            <div className="nav-right">
              ...
              {isGuest && <button className="btn btn-secondary btn-sm" onClick={() => setCurrentUser(null)}>Załóż konto</button>}
              <button className="btn btn-ghost" style={{ border: "1px solid rgba(255,255,255,.25)", borderRadius: 8 }} onClick={() => { setCurrentUser(null); setCart([]); setPage("shop"); }}>{isGuest ? "Zakończ" : "Wyloguj"}</button>
            </div>
          </div>
        </nav>
        ...
        <main className="main">
          ...
          {page === "shop" && <ShopPage products={filtered} categories={cats} categoriesFull={categories} filterCat={filterCat} setFilterCat={setFilterCat} filterSubcat={filterSubcat} setFilterSubcat={setFilterSubcat} searchQ={searchQ} setSearchQ={setSearchQ} onAdd={addToCart} discount={discount} />}
          {page === "csv" && isAdmin && <CsvImportPage products={products} setProducts={setProducts} showAlert={showAlert} setLastSync={setLastSync} />}
          {page === "products" && isAdmin && <AdminProducts products={products} setProducts={setProducts} categories={categories} setCategories={setCategories} showAlert={showAlert} modal={modal} setModal={setModal} editItem={editItem} setEditItem={setEditItem} />}
          ...
          {page === "orders" && !isGuest && <OrdersPage orders={isAdmin ? orders : orders.filter(o => o.userId === currentUser.id)} isAdmin={isAdmin} />}
        </main>
      </div>
...
function ShopPage({ products, categories, categoriesFull, filterCat, setFilterCat, filterSubcat, setFilterSubcat, searchQ, setSearchQ, onAdd, discount }) {
...
function CsvImportPage({ products, setProducts, showAlert, setLastSync }) {
  ...
  const buildPreview = () => {
    ...
    const rows = rawData.map((row, idx) => {
      ...
      let status = "new";
      if (existing) {
        status = (existing.price !== price || existing.stock !== stock || existing.name !== name) ? "update" : "unchanged";
      }
      return { idx, sku, name, category, price, stock, description, status, existing };
  ...
  const doImport = () => {
    ...
    const res = { added, updated, unchanged, total: preview.length };
    setResult(res);
    setLastSync({ file: fileName, imported: added + updated, time: new Date().toLocaleTimeString("pl-PL") });
    showAlert(`✅ Import zakończony: ${added} nowych, ${updated} zaktualizowanych`, "success");
    setStep(4);
  };

  const reset = () => { setStep(1); setRawData(null); setCsvHeaders([]); setPreview([]); setResult(null); setFileName(""); };
...
SLU-002;Słuchawki Bluetooth;Elektronika;243,09;299,00;30;Opis produktu;szt`}</div>
            ...
            <table className="preview-table">
              <thead>
                <tr>
                  ...
                  <th>Cena CSV</th><th>Cena bieżąca</th><th>Stan CSV</th><th>Stan bieżący</th>
                </tr>
              </thead>
              <tbody>
                {preview.map(row => (
      ...
      {step === 4 && result && (
        <div className="card" style={{ textAlign: "center" }}>
          ...
          <p className="text-muted text-sm" style={{ marginBottom: 20 }}>Plik: <strong>{fileName}</strong> · {result.total} wierszy przetworzonych</p>
...
// ── ADMIN PRODUKTY ────────────────────────────────────────────────────────────
function AdminProducts({ products, setProducts, categories, setCategories, showAlert, modal, setModal, editItem, setEditItem }) {
  const [form, setForm] = useState({ name: "", category: categories[0]?.name || "", subcategory: "", price: "", stock: "", weight: "", unit: "szt", image: "📦", description: "", sku: "" });
  const [newCat, setNewCat] = useState("");
  const [newSub, setNewSub] = useState({});
  const [catFilter, setCatFilter] = useState(categories[0]?.name || "");
  const emojis = ["📦", "💻", "🎧", "📱", "👕", "👟", "🍕", "☕", "📘", "🎒", "⌚", "🏋️", "🎮", "🌿"];

  const openAdd = () => { setForm({ name: "", category: categories[0]?.name || "", subcategory: "", price: "", stock: "", weight: "", unit: "szt", image: "📦", description: "", sku: "" }); setEditItem(null); setModal("product"); };
  const openEdit = (p) => { setForm({ ...p, subcategory: p.subcategory || "", price: String(p.price), stock: String(p.stock), weight: String(p.weight || ""), unit: p.unit || "szt" }); setEditItem(p); setModal("product"); };
  const save = () => {
          ...
          <table>
            ...
            <tbody>{products.map(p => (
              <tr key={p.id}>
                ...
                <td><span style={{ fontSize: "1.2rem", marginRight: 7 }}>{p.image}</span><strong>{p.name}</strong></td>
      ...
      {modal === "product" && (
        <div className="modal-bg">
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-group"><label className="form-label">Ikona</label>
              ...
              <div className="form-group"><label className="form-label">Opis</label><input className="form-input" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} /></div>
              <div className="flex gap-3">
...
// ── ZAMÓWIENIA ────────────────────────────────────────────────────────────────
function OrdersPage({ orders, isAdmin }) {
  return (
    <>
          ...
          <div key={o.id} className="card" style={{ marginBottom: 14 }}>
            ...
            <div className="table-wrap">
              <table>
                ...
                <tbody>{o.items.map(i => <tr key={i.id}><td>{i.image} {i.name}</td><td>{fmt(i.price)}</td><td>{i.qty} {UNITS.find(u => u.value === i.unit)?.label || "szt."}</td><td className="font-bold">{fmt(i.price * i.qty)}</td></tr>)}</tbody>
              </table>
            </div>
